...
const RegisterModal = () => {
	...
	return (
		<div
			className={cx(
				'fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-gray-500/50 p-5 transition duration-300',
		...
		>
			...
			<Form
				...
				render={({ handleSubmit }) => (
					<form className='relative mx-auto w-full max-w-lg space-y-4 rounded bg-white p-5' onSubmit={handleSubmit}>
						<h1>Sign Up</h1>
						...
						<div className='flex items-center justify-end gap-2'>
							<button
								type='button'
								className='rounded border border-gray-400 px-3 py-2 text-sm font-medium text-gray-900'
								onClick={closeModal}
							>
								Cancel
							</button>
							<button type='submit' className='rounded bg-indigo-500 px-3 py-2 text-sm font-medium text-white'>
								Register
							</button>
						</div>
					</form>
